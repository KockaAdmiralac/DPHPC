...
__global__ void kernel_3(const int n, const DATA_TYPE alpha, const DATA_TYPE *A, DATA_TYPE *w, const DATA_TYPE *x) {
    ...
    const int smem_cols_per_thread = 8;  // this and threads per block are basically manually found best performance.
    ...
    for (int row_block = blockIdx.x * blockDim.x; row_block < _PB_N; row_block += gridDim.x * blockDim.x) {
        ...
        for (int col_block = 0; col_block < _PB_N; col_block += smem_cols_per_thread) {
            ...
            // for (int slot_local_row = 0; slot_local_row < rows_in_block; slot_local_row++) {
            //     for (int slot_local_col = threadIdx.x; slot_local_col < cols_in_block;
            ...
            //         smem_A[slot_local_col*rows_in_block+slot_local_row] = A[from_A_row * n + from_A_col];
            //     }
            // }
            ...
            if (row < n) {
                for (int j = 0; j < cols_in_block; j++) {
                    // DATA_TYPE new_val = smem_A[j * rows_in_block + threadIdx.x] * x[col_block + j];
                    // DATA_TYPE new_val = smem_A[threadIdx.x * cols_in_block + j] * x[col_block + j];
                    w_inc += smem_A[j * rows_in_block + threadIdx.x] * x[col_block + j];
