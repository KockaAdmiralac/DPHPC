...
// Based on serial_block_first_loop.
void kernel_gemver(int n, DATA_TYPE alpha, DATA_TYPE beta, DATA_TYPE POLYBENCH_2D(A, N2, N2, n, n),
    ...
    for (int j = 0; j < _PB_N; j += CACHE_SIZE)
        for (int i = 0; i < _PB_N; ++i) {
            const __m256d u1i = _mm256_set1_pd(u1[i]);
            const __m256d u2i = _mm256_set1_pd(u2[i]);
            for (int bj = j; bj < j + CACHE_SIZE && bj < _PB_N; bj += 4) {
                _mm256_store_pd(&A[i][bj], _mm256_fmadd_pd(u2i, _mm256_load_pd(&v2[bj]),
                                                           _mm256_fmadd_pd(u1i, _mm256_load_pd(&v1[bj]),
                                                                           _mm256_load_pd(&A[i][bj]))));
            }
        }
